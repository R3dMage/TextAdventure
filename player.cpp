...
Player::Player()
{
	...
	plot.Priest4 = false;
	plot.Monk = false;
	plot.GreenDragon = false;
	plot.BlueDragon = false;
	plot.RedDragon = false;
	plot.Start = false;
	Loaded = false;
	music = true;
}
...
void Player::Save(ofstream &fout)
{
	...
	fout << "Grn: "		<< plot.GreenDragon << endl;
	fout << "Blu: "		<< plot.BlueDragon << endl;
	fout << "Red: "		<< plot.RedDragon << endl;
...
void Player::Load(ifstream &fin)
{
	...
	fin >> temp >> plot.GreenDragon;
	fin >> temp >> plot.BlueDragon;
	fin >> temp >> plot.RedDragon;
	fin >> temp >> plot.Monk;
	fin >> temp >> music;
	weap = loadWeapon(W);
	arm = loadArmor(A);
